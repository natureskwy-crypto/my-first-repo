...
exports.fasstoDataFetcher = async (req, res) => {
  ...
  try {
    ...
    // Determine dates for API call
    const today = new Date();
    const yyyy = today.getFullYear();
    const mm = String(today.getMonth() + 1).padStart(2, '0');
    const dd = String(today.getDate()).padStart(2, '0');

    // Use provided dates from Apps Script payload, or default to today's date.
    const startDate = reqStartDate || `${yyyy}-${mm}-${dd}`;
    const endDate = reqEndDate || `${yyyy}-${mm}-${dd}`;
    ...
    // Ensure the service account (e.g., 86959409290-compute@developer.gserviceaccount.com)
    // has the 'Google Sheets Editor' role (roles/sheets.editor) in IAM & Admin.
