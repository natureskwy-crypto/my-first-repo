...
// allowed origins comma separated, default to only Apps Script
...
// Google Sheets client (reuse across invocations)
...
// Fassto authentication with retry
async function getAccessToken() {
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      structuredLog({
        ...
        attempt,
        status: 'success',
        message: 'Authenticated to Fassto',
        responseHeader: res.data?.header || null,
  ...
  throw new Error('Unexpected authentication failure'); // should not reach
}

// core logic (used by both Cloud Function and Express)
async function fasstoDataFetcherImpl(req, res) {
  ...
  try {
    ...
    if (!spreadsheetId || !sheetName) {
      structuredLog({ traceId, stage: 'validate', severity: 'ERROR', message: 'Missing spreadsheetId or sheetName', spreadsheetId, sheetName });
    ...
    while (attempt < maxDeliveryAttempts) {
      try {
        ...
        structuredLog({
          ...
          status: 'success',
          url: deliveryUrl,
          responseHeader: deliveryRes.data?.header || null
        });
        break;
      } catch (err) {
        const status = err.response?.status || null;
    ...
    if (items.length === 0) {
      structuredLog({ traceId, stage: 'no_data', message: 'No delivery items', usedFallbackDates, deliveryRaw: deliveryRes.data });
    ...
    return res.status(200).json({
      traceId,
      status: 'success',
      message: `Recorded ${rows.length} delivery items`,
      count: rows.length,
      spreadsheetId,
      range: safeRange,
      usedFallbackDates
    ...
    const apiMsg = err.response?.data?.header?.msg || err.message || 'Unknown error';
    structuredLog({
      stage: 'fatal',
...
if (require.main === module) {
  ...
  // CORS preflight generic (delegated inside handler)
  app.options('*', (req, res) => {
    const origin = req.headers.origin || '';
    if (ALLOWED_ORIGINS.includes(origin)) {
      res.set('Access-Control-Allow-Origin', origin);
  ...
  app.listen(port, () => {
    structuredLog({ stage: 'server_start', message: `Listening on ${port}` });
  });
}
