 ...
 * @version 2.5.0 (Test with a one-month date range)
 */
...
exports.fasstoDataFetcher = async (req, res) => {
  ...
  try {
    ...
    const startDateObj = new Date(kstNow);
    startDateObj.setMonth(startDateObj.getMonth() - 1);
    const startDate = startDateObj.toISOString().slice(0, 10);

    console.log(`[TESTING] 날짜를 최근 1달(${startDate} ~ ${endDate})로 설정하여 테스트합니다.`);
